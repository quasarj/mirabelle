...
function setupVolPanel() {
  ...
  const elementList = [volAxialContent, volSagittalContent, volCoronalContent];
  ...
  return {
    axial: volAxialContent,
    sagittal: volSagittalContent,
    coronal: volCoronalContent,
  };
}
...
function setupMipPanel() {
  ...
  const elementList = [mipAxialContent, mipSagittalContent, mipCoronalContent];
  ...
  return {
    axial: mipAxialContent,
    sagittal: mipSagittalContent,
    coronal: mipCoronalContent,
  };
}
...
function setupVolTools() {
  ...
  // Tool group setup
  const volToolGroup =
    cornerstoneTools.ToolGroupManager.createToolGroup("vol_tool_group");
  ...
  const volVOISyncronizer =
    cornerstoneTools.synchronizers.createVOISynchronizer("vol_voi_syncronizer");
...
function setupMipTools() {
  // Tool group setup
  const mipToolGroup =
    cornerstoneTools.ToolGroupManager.createToolGroup("mip_tool_group");
  ...
  mipToolGroup.setToolActive(
    cornerstoneTools.VolumeRotateMouseWheelTool.toolName,
  );

  // Segmentation Display
  mipToolGroup.addTool(cornerstoneTools.SegmentationDisplayTool.toolName);
  mipToolGroup.setToolEnabled(
    cornerstoneTools.SegmentationDisplayTool.toolName,
  );
  ...
  const mipVOISyncronizer =
    cornerstoneTools.synchronizers.createVOISynchronizer("mip_voi_syncronizer");
...
function setup3dTools() {
  // Tool group setup
  const t3dToolGroup =
    cornerstoneTools.ToolGroupManager.createToolGroup("t3d_tool_group");
  ...
  addDropDownToToolbar({
    ...
    labelText: "Preset: ",
    options: {
      values: cornerstone.CONSTANTS.VIEWPORT_PRESETS.map(
        (preset) => preset.name,
      ),
  ...
  if (fileID) {
    ...
    if (timepointID) {
      response = await fetch(
        `/papi/v1/series/${seriesUID}:${timepointID}/files`,
      );
    } else {
    ...
    if (response.ok) {
      ...
      fileList = files.file_ids.map(
        (file_id) => `wadouri:/papi/v1/files/${file_id}/data`,
      );
    }
  }
