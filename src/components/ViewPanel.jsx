...
function ViewPanel({ files, volumeName, iec }) {
  ...
  return (
    <div
      id="viewPanel"
      className="grid grid-rows-[1fr,auto] text-center gap-2 overflow- h-full"
    >
      {viewport_layout == "stack" && files && files.length > 0 && (
        <ViewStackPanel files={files} volumeName={volumeName} iec={iec} />
      )}
      {viewport_layout == "volume" && files && files.length > 0 && (
        <ViewVolumePanel files={files} volumeName={volumeName} iec={iec} />
      )}
    </div>
  );
}
