...
function MainPanel({ details, files, iec, iecs }) {
  ...
  return (
    <div
      id="app"
      className={`relative grid grid-rows-[auto,1fr] gap-2 w-screen min-w-[1300px] h-screen p-2 dark:bg-blue-950 overflow-hidden`}
    >
      <Header />
      {topPanelVisible && <TopPanel />}
      <div
        id="main"
        className={`h-full w-full grid ${gridTemplate} rounded-lg gap-2 transition-all duration-200 overflow-hidden`}
      >
        <div
          id="leftPanel"
        ...
        <MiddlePanel files={files} iecs={iecs} iec={iec} />
        <div
          id="rightPanel"
          className="w-full h-full rounded-lg overflow-hidden"
        >
      ...
      >
        <span className="material-symbols-rounded rounded-full leading-5 text-white">
          chevron_left
        </span>
      </button>
      <button
